...
class AlphasGurobiBased:
    def __init__(self, rnnModel, xlim, update_strategy_ptr=Absolute_Step):
        ...
        self.rnn_output_idxs = rnn_output_idxs + rnn_output_idxs
        ...
        assert len(self.alphas) == len(self.rnn_output_idxs)
    ...
    def build_gurobi_query(self, alphas_sum=None):
        ...
        for t in range(1, self.n_iterations):
            for i in range(self.w_h.shape[0]):
                ...
                gmodel.addConstr(alphas_u[i] * t >= cond_u + cond_x, "alpha_u{}_t{}".format(i, t))
                gmodel.addConstr(alphas_l[i] * t <= cond_l + cond_x, "alpha_l{}_t{}".format(i, t))
        ...
        if gmodel.status == GRB.CUTOFF or gmodel.status == GRB.INFEASIBLE:
            ...
            return None
        # print("FEASIBLE sum_alpahs = {} constraint_type={}".format(alphas_sum, ''))
    ...
    def do_step(self, strengthen=True, invariants_results=[]):
        ...
        if self.same_step_counter > 10:
            ...
            new_alphas = self.build_gurobi_query(alphas_sum=new_min_sum)
